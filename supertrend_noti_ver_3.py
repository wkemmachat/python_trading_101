...
'''
schedule.every(60*60).seconds.do(run_bot)
while True:
    schedule.run_pending()
    # msg = 'Running Bot: 104 '+strategy+', Timeframe: '+timeframe+', Timing : '+get_date_time()
    # r = requests.post(url_line, headers=headers, data = {'message':msg})
    # print (msg)
    # print (r.text)
    time.sleep(1)
...
# print(buy_signal_symbols)
# print(sell_signal_symbols)
# print(no_signal_symbols)


# for symbol_in_loop in range(len(buy_signal_symbols)):
#    print('Buy Symbol :'+ symbol_in_loop)

# for symbol_in_loop in range(len(sell_signal_symbols)):
#    print('Sell Symbol :'+ symbol_in_loop) 


symbol_cut_usdt_list = []
for symbol_in_loop in no_signal_symbols:
    # print(symbol_in_loop)
    symbol_cut_usdt = str(symbol_in_loop).partition('/')[0]
    if symbol_cut_usdt in symbol_cut_usdt_list:
        continue
    symbol_cut_usdt_list.append(symbol_cut_usdt)

symbol_new_line = ''
for symbol_in_loop in symbol_cut_usdt_list:
    symbol_new_line += symbol_in_loop+'\n'

    
# saving the dataframe
print(symbol_cut_usdt_list)
print(type(symbol_cut_usdt_list))
filename = 'symbol_no_signal_'+get_date()+'.csv'
# df.to_csv(filename) 
with open(filename, 'w') as f:
    ...
    writer.writerow(symbol_cut_usdt_list)


msg = '-- No Signal --, Timing : '+get_date_time()+', symbol : \n'+ symbol_new_line
r = requests.post(url_line, headers=headers, data = {'message':msg})
...
'''
symbol_new_line = ''
for symbol_in_loop in buy_signal_symbols:
    # print(symbol_in_loop)
    symbol_new_line += symbol_in_loop+'\n'


msg = '-- Buy Signal --, Timing : '+get_date_time()+', symbol : \n'+ symbol_new_line
r = requests.post(url_line, headers=headers, data = {'message':msg})

symbol_new_line = ''
for symbol_in_loop in sell_signal_symbols:
    # print(symbol_in_loop)
    symbol_new_line += symbol_in_loop+'\n'

msg = '-- Sell Signal --, Strategy: '+strategy+', Timing : '+get_date_time()+', symbol : \n'+ symbol_new_line
r = requests.post(url_line, headers=headers, data = {'message':msg})
