...
### LINE NOTI
token_line = 'UYs5Oh0SRgwwhgrCI9tsAHwEKK1NybMZCChHVymLNGV'
...
try:
    ...
    msg = 'API Kucoin Working -- Normal --, Timing : '+get_date_time()
    # r = requests.post(url_line, headers=headers, data = {'message':msg})
    # print (r.text)
except:
    msg = 'API Kucoin --- ERROR ---, Timing : '+get_date_time()
    # r = requests.post(url_line, headers=headers, data = {'message':msg})
    # print (r.text)
    canContinue = False
...
all_symbols_df = pd.read_csv('symbol_kucoin.csv')
...
for index, row in all_symbols_df.iterrows():
    ...
    try:
        ...
        msg = 'API Kucoin --- ERROR ---, Timing : '+get_date_time()+', Symbol :'+symbol
        r = requests.post(url_line, headers=headers, data = {'message':msg})
        # print (r.text)
        canContinue = False
