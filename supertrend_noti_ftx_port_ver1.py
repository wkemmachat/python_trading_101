...
### LINE NOTI
token_line = 'GdXixdPdJ2gSxvy9xNS06qMkBgukBL4ew7JJ9GYs18H'
...
try:
    ...
    msg = 'API FTX Working -- Normal --, Timing : '+get_date_time()
    # r = requests.post(url_line, headers=headers, data = {'message':msg})
    # print (r.text)
except:
    msg = 'API FTX --- ERROR ---, Timing : '+get_date_time()
    # r = requests.post(url_line, headers=headers, data = {'message':msg})
    # print (r.text)
    canContinue = False
...
all_symbols_df = pd.read_csv('symbol_ftx.csv')
...
for index, row in all_symbols_df.iterrows():
    ...
    try:
        ...
        msg = 'API FTX --- ERROR ---, Timing : '+get_date_time()+', Symbol :'+symbol
        r = requests.post(url_line, headers=headers, data = {'message':msg})
        # print (r.text)
        canContinue = False
